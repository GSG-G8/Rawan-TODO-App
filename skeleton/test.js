...
test ('Testing addTodo(), it should return the array plus the new object', function () {
  ...
  const newTodo = {id:4, description:"Forth task", done:false};
  
  const actual = todoFunctions.addTodo(todoArr, newTodo);
  
  const expected = [
      {id:1, description: "First task", done:false},
      {id:3, description: "Second task", done:false},
      {id:2, description: "Third task", done:false},
      {id:4, description:"Forth task", done:false}];
  ...
  test ('Testing markTodo(), it should change between done and not done yet', () =>{
    const todoArr = [{id:10, description:"Eating Breakfast", done:false}, {id:15, description:"Visting my grandma", done:true}];
    const actual = todoFunctions.markTodo(todoArr, 10);
    const expected = [{id:10, description:"Eating Breakfast", done:true}, {id:15, description:"Visting my grandma", done:true}];
    expect(actual).toEqual(expected);
  });
  ...
  test ('Testing markTodo(), it should change between done and not done yet', () =>{
    ...
    const expected = [{id:10, description:"Eating Breakfast", done:false}, {id:15, description:"Visting my grandma", done:false}];
    expect(actual).toEqual(expected);
  });
